...
- Latency in claude_cli mode, measured on Berk's M3 Air: from end of speech to first word went from about 3.3 s to 1.4-1.6 s. One CLI process stays alive across turns (`--input-format stream-json`), started at the wake word if none is running and closed after `llm.cli_session_idle_s` (900 s) or `llm.history_turns` turns; a new session gets recent turns from memory in its first message. Prewarming a plain `claude -p` did not help: the CLI only initialises after reading its prompt and exits if stdin stays empty for 3 s, which caused random "something went wrong" replies. It runs with `--setting-sources ""` from ~/.voiceagent (so Berk's own hooks, plugins and this CLAUDE.md don't load), and with `MAX_THINKING_TOKENS=0` (Haiku's extended thinking cost about 0.7 s). The rest is Whisper base (about 0.6 s) and model time to first token (about 0.9 s). Whisper small was tested: better Turkish but 2.9 s per sentence on this Mac, so base stays.
...
- Google Calendar and Gmail (`tools/google.py`): Berk's own Google Cloud OAuth client (Desktop app) and `google-login client_secret.json`, tokens in ~/.voiceagent/google.json. Tools `calendar` (list, create, delete, in the calendar's time zone) and `gmail` (search, read, draft, send; send only after the user confirms a read-back). Chosen over claude.ai's Gmail/Calendar connectors so the data doesn't depend on which Claude account the CLI uses and works in API mode too. Unit tested against a fake API only.
...
- `install.ps1`: Windows (PowerShell) equivalent of install.sh; the brain runs natively on Windows (not WSL) so the phone can reach it on the LAN. The brain needs no PortAudio or `say` (sounddevice is imported lazily only in satellite/run mode). `install-service` supports Windows via a Task Scheduler logon job (restart on failure); admin restart spawns a fresh process on Windows instead of os.execv.
